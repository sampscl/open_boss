defmodule Mix.Tasks.OpenBoss.BuildSelfSignedKeys do
  ...
  def run(_) do
    ...
    open_boss_key = X509.PrivateKey.new_ec(:secp256r1)
    ...
    open_boss_cert =
      ...
      |> X509.Certificate.new(
        ...
        ca_key,
        extensions: [
          subject_alt_name: X509.Certificate.Extension.subject_alt_name([subject_an])
        ]
      )
    ...
    :ok = File.write!("etc/ssl/private/ca_key.pem", X509.PrivateKey.to_pem(ca_key))
    :ok = File.chmod("etc/ssl/private/ca_key.pem", 0o0400)

    :ok = File.write!("etc/ssl/certs/ca_crt.pem", X509.Certificate.to_pem(ca_cert))
    :ok = File.chmod("etc/ssl/certs/ca_crt.pem", 0o0440)

    :ok = File.write!("etc/ssl/private/open_boss_key.pem", X509.PrivateKey.to_pem(open_boss_key))
    :ok = File.chmod("etc/ssl/private/open_boss_key.pem", 0o0400)

    :ok = File.write!("etc/ssl/certs/open_boss_crt.pem", X509.Certificate.to_pem(open_boss_cert))
